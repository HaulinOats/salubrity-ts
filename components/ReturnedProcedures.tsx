...
interface ReturnedProceduresProps {
  completedCalls: Call[];
  editCompletedCall: (
    callId: string,
  ...
  updateCompletedCalls: (calls: Call[]) => void;
  insertionAgg?: any;
  user: User;
  hideUI?: () => void;
}

const ReturnedProcedures: React.FC<ReturnedProceduresProps> = (props) => {
  ...
  const aggregateData = () => {
    let completedCalls = props.completedCalls;
    let hospitalObj: { [key: number]: { count: number; text: string } } = {};
    completedCalls.forEach((call) => {
      if (hospitalObj[call.hospital!]) {
        hospitalObj[call.hospital!].count++;
      } else {
        hospitalObj[call.hospital!] = {
          text: refData.hospitalsById![call.hospital!].name,
          count: 0,
        };
      }
    });
  ...
  const sortByOnChange = (e: ChangeEvent<HTMLSelectElement>) => {
    let completedCalls = props.completedCalls;
    if (e.target.value !== "callTime") {
      ...
      props.updateCompletedCalls(completedCalls);
    } else {
      completedCalls.sort((a: Call, b: Call) => {
        if (
  ...
  const toggleShowRecord = (e: React.MouseEvent<HTMLParagraphElement>) => {
    let showToggleContainer = (e.target as HTMLParagraphElement).closest(
    ...
    showToggleContainer?.nextElementSibling?.classList.toggle("vas-hide");
  };
  ...
  return (
    <div className="vas-table vas-returned-procedures-table">
        ...
        <span>
          <div className="vas-returned-procedures-aggregations">
            <p className="vas-returned-procedures-records-returned">
              {props.completedCalls.length} Records Returned
            </p>
            <div className="vas-returned-procedures-aggregations-insertions-container">
              {props.insertionAgg &&
                props.insertionAgg.map((insertion: any) => {
                  return (
                    <div
                      key={insertion.index}
                    ...
                    >
                      <p className="vas-returned-procedures-aggregations-insertions-item-left">
                        {refData.itemsById![insertion.itemId].value}
                      </p>
      ...
      {!props.hideUI && props.completedCalls.length > 0 && (
                      ...
                      <div className="vas-admin-custom-table-item vas-call-table-item">
                        ...
                        <div
                          className={
                            ...
                            (call.orderChange ? "vas-admin-order-change" : "")
                          }
                        >
                          <div className="vas-admin-custom-table-td vas-admin-custom-table-nurse">
                            ...
                            <p className="vas-admin-custom-item-subvalue">
                              {refData.usersById![call.completedBy!].fullname ??
                                call.completedBy}
                            </p>
                          </div>
                          ...
                          <span>
                            <div className="vas-admin-custom-table-td vas-admin-custom-table-hospital">
                              ...
                              <p className="vas-admin-custom-item-subvalue">
                                {refData.hospitalsById![call.hospital!].name ??
                                  "N/A"}
                              </p>
                            </div>
                            {call.hospital === 6 &&
                            ...
                            {call.orderChange && (
                              <div className="vas-admin-custom-table-td vas-admin-custom-table-order-change">
                                ...
                                <p className="vas-admin-custom-item-subvalue">
                                  {
                                    refData.orderChangeById![call.orderChange]
                                      .name
                                  }
                                </p>
                              </div>
                        ...
                        <div className="vas-home-custom-table-column-3">
                          <div className="vas-call-times-row">
                            <p className="vas-call-times-left">Call Time:</p>
                            <p className="vas-call-times-right">
                              <Moment format="HH:mm">{call.createdAt}</Moment>
                            </p>
                          </div>
                          <div className="vas-call-times-row">
                            <p className="vas-call-times-left">Start Time:</p>
                            <p className="vas-call-times-right">
                              <Moment format="HH:mm">{call.startTime}</Moment>
                            </p>
                          </div>
                          <div className="vas-call-times-row">
                            <p className="vas-call-times-left">End Time:</p>
                            <p className="vas-call-times-right">
                              <Moment format="HH:mm">{call.completedAt}</Moment>
                            </p>
                          </div>
                          <div className="vas-call-times-row">
                                      ...
                                      return (
                                        ...
                                        >
                                          <p className="vas-returned-procedures-query-item-left">
                                            {refData.itemsById![id]
                                              .groupName !==
                                            refData.itemsById![id].value
                                              ? refData.itemsById![id]
                                                  .groupName + ":"
                                              : ""}
                                          </p>
...
export default ReturnedProcedures;
